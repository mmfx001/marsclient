    ...
    return (
        <div className="p-6 bg-gray-100 min-h-screen">
            ...
            {status === 'loading' ? (
                ...
                <ul className="space-y-6 flex flex-wrap items-center gap-6 justify-center">
                    {purchasedProducts.map((product, index) => {
                        // Redux store'dan topilgan mahsulot
                        const foundProduct = products.find(p => p.id === product.id);
                        return (
                            <li key={index} className="border w-[45%] flex-col gap-5 border-gray-300 rounded-lg p-6 shadow-md bg-white hover:shadow-lg transition-shadow duration-300 flex items-center">
